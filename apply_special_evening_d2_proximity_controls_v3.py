...
def _decode(path: Path):
    ...
    try:
        ...
        raise PatchError(f"{path}: cannot decode as {encoding}: {exc}") from exc

    if "\x00" in text:
        raise PatchError(
            f"{path}: decoded text still contains NUL characters; "
            "stop before changing the file"
        )
...
def read(path: str) -> str:
    ...
    if p not in STAGED:
        ...
        text, encoding, newline = _decode(p)
        STAGED[p] = text
        META[p] = (encoding, newline)
...
for file_path, text in STAGED.items():
    encoding, newline = META.get(file_path, ("utf-8", "\n"))
    ...
    line_mode = "CRLF" if newline == "\r\n" else "LF"
    print(f"[WRITE] {file_path.relative_to(ROOT)} ({encoding}, {line_mode})")
