...
def read(path):
    ...
    if p not in STAGED:
        ...
        text, encoding, newline = _read_text(p)
        STAGED[p] = text
        ENCODINGS[p] = encoding
        NEWLINES[p] = newline
    return STAGED[p]
...
def ensure_absent(path, marker, label):
...
try:
    if "class HomeProximity(str, Enum):" not in read("backend/presentation/models.py"):
        raise PatchError("D1 HomeProximity foundation is missing")
...
for file_path, text in STAGED.items():
    ...
    newline = NEWLINES.get(file_path, "\n")
    rendered = text.replace("\n", newline)

    if file_path == ROOT / "frontend/scenes/scene-map.js":
        encoding = "utf-8-sig"

    if encoding == "utf-16":
        raw = rendered.encode("utf-16")
    elif encoding == "utf-8-sig":
        raw = rendered.encode("utf-8-sig")
    else:
    ...
    mode = "CRLF" if newline == "\r\n" else "LF"
    print(f"[WRITE] {file_path.relative_to(ROOT)} ({encoding}, {mode})")

print()
print("D2 applied: explicit Special Evening proximity controls.")
print("Review: git diff --text")
print()
print(r"Python: .\.venv\Scripts\python.exe -m pytest tests/test_presentation_runtime.py -q")
print(r"Frontend: node frontend\renderer\special-evening-proximity.test.cjs")
