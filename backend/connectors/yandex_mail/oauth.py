...
def authorize(*,client_id,client_secret,authorization_code=None,code_prompt=input,prompt_open=webbrowser.open,policy_store=None,safety_store=None):
 assert_yandex_mail_network_allowed(policy_store=policy_store,safety_store=safety_store); value=verifier(); prompt_open("https://oauth.yandex.ru/authorize?"+urlencode({"response_type":"code","client_id":client_id,"redirect_uri":REDIRECT_URI,"scope":YANDEX_MAIL_SCOPE,"code_challenge":challenge(value),"code_challenge_method":"S256"}))
