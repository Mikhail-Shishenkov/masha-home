...
_FIRST_PERSON_MUTATION = re.compile(
    ...
    r"запланирова(?:л|ла|ли|но)"
    r")\b",
    re.IGNORECASE,
)
_RESULT_STATE_CLAIM = re.compile(
    r"\b(?:дело|задач[аиу]?|обязательство|напоминание|запись|память|событие|"
    r"занятие|встреча|документ|файл|письмо)\b"
    r".{0,80}\b(?:создан[ао]?|добавлен[ао]?|сохранен[ао]?|записан[ао]?|"
    r"изменен[ао]?|обновлен[ао]?|удален[ао]?|скрыт[ао]?|завершен[ао]?|"
    r"выполнен[ао]?|отправлен[ао]?|запланирован[ао]?)\b",
...
_SUCCESS_MUTATION_SEQUENCE = re.compile(
    ...
    r"заб(?:ыл|ыла|ыли|ыто)|удал\w*|скр\w*|заверш\w*|закр\w*|выполн\w*|"
    r"отправ\w*|запланир\w*"
    r")\b",
    re.IGNORECASE | re.DOTALL,
)
...
_EXECUTION_MUTATION_FORM = re.compile(
    ...
    r"запланир(?:ую|уешь|ует|уем|уете|уют|овал(?:а|и)?|овано)|"
...
UNRECEIPTED_MUTATION_RESPONSE = (
    "Я пока ничего не меняла — не хочу делать вид, будто действие уже выполнено. "
    "Уточни, что именно ты хочешь поручить, и я честно скажу, могу ли это сделать."
)
...
_APPLICATION_STATE_NOUN = re.compile(
    ...
    r"событи\w*|заняти\w*|встреч\w*|документ\w*|файл\w*|письм\w*|"
    r"нить|нити|тему|темы|приложени\w*|календар\w*)\b",
    re.IGNORECASE,
)
