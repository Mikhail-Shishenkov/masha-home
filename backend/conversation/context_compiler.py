...
BEHAVIORAL_CONTRACT = (
    ...
    "полной памяти. 3) Если нужной записи нет в bounded context, не угадывай и "
    "скажи, что сейчас недостаточно контекста; не утверждай, что такая запись "
    "никогда не существовала. 4) В этом ходе нет Application Receipt об изменении состояния. "
    "Поэтому не утверждай ни в какой формулировке, что уже изменила память или "
    "состояние приложения. Обсуждение намерения и предложение пользователю "
    "подтвердить операцию допустимы, но не являются выполнением. Например, "
    "не говори «я запомнила», когда подтверждённого receipt нет. "
