...
class ControlledProactiveRuntime:
    ...
    def run_checkin_cycle(self, policy: ProactivePolicy, *, reminder_pending: bool | None = None) -> ProactiveCycleResult:
        ...
        if reminder_pending is None:
            reminder_pending = any(
                (record := self.interaction_store.get(item.event_id)) is None
                or record["state"] in {"candidate", "delivered"}
    ...
    def record_decision(self, event_id: str, decision: str, reason: str, *, entity_type: str) -> None:
        ...
        if previous and previous[-1]["payload"] == payload:
            return
        self.repository.record_event(action="proactive_decision", entity_type=entity_type, entity_id=event_id, payload=payload)
