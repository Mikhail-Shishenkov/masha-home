...
_EXPLICIT = (
    ...
    re.compile(
        ...
        r"(?:у|по|про)\s*(?P<query>.*)$"
    ),
    re.compile(
        r"^(?:найди|поищи|посмотри|проверь)\s+(?:новости|новое)\s+"
        r"(?:у|по|про)\s*(?P<query>.*)$"
    ),
    re.compile(
        r"^ты\s+(?:писала|говорила|думала).+?\s+(?:найди|поищи|посмотри|проверь)\s*,?\s*"
        r"(?:что\s+)?нового\s+(?:у|по|про)\s*(?P<query>.*)$"
    ),
    re.compile(
        r"^ты\s+(?:писала|говорила|думала).+?\s+(?:найди|поищи|посмотри|проверь)\s*,?\s*"
        r"(?:что\s+)?у\s+(?P<query>.+?)\s+нового$"
    ),
)
