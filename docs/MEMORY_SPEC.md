...
- пользовательское представление использует настроенный Home timezone;
- текущий default Home timezone репозитория — `Europe/Saratov`;
...
Human availability, search/recall и различие ARCHIVED/FORGOTTEN нормативно
определены в [HUMAN_INFORMATION_MODEL_V0.3.1.md](HUMAN_INFORMATION_MODEL_V0.3.1.md).
Legacy operation `archive` может сохранять старое поведение скрытия для
совместимости, но не является источником human ARCHIVED: human archive выводится
из доменного lifecycle.
